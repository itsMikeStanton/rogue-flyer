...
post.setHudCanvas(hud.canvas); // let the grade pass warp + composite the HUD onto the CRT tube
// While the composer owns the frame it draws the HUD itself (warped); hide the
// flat CSS overlay so it isn't drawn twice. Change-detected to avoid DOM churn.
...
// The opening boom: a violent burst of lava thrown high + a volley of bombs.
function lavaBurst() { for (let i = 0; i < 60; i++) spawnSpray(1.6); for (let i = 0; i < 7; i++) spawnLavaBomb(); }
...
function updateEruption(dt) {
  ...
  // not a clean pulse.
  const now = performance.now();
  const flick = Math.sin(now * 0.013) * 0.45 + Math.sin(now * 0.043 + 1.1) * 0.32 + Math.sin(now * 0.101 + 2.7) * 0.23 + (Math.random() < 0.05 ? Math.random() * 1.6 : 0);
  e.t += dt;
...
function frame(now) {
  ...
  if (!inXR && post.enabled) {
    setHudPath(true); // composer warps the HUD onto the tube; CSS overlay hidden
    try { post.render(dt); }
